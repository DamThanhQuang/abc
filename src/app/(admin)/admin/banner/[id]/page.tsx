...
import { updateHeroSlide } from "@/lib/actions/hero-slides";
import { getHeroSlideById } from "@/lib/api/hero-slides";
import { requireAdmin } from "@/lib/auth-guard";
import { heroSlideInputFromForm } from "@/lib/hero-slide-form";
import { idSchema } from "@/lib/validations";

export const metadata: Metadata = { title: "Sửa ảnh banner | Admin" };
...
export default async function AdminEditBannerPage({
  ...
  async function handleUpdate(formData: FormData) {
    ...
    const result = await updateHeroSlide(id, heroSlideInputFromForm(formData));
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="font-heading font-bold text-[22px] text-content-heading">Sửa ảnh banner</h1>
            <p className="font-sans text-[13px] text-content-muted">{slide.imageAlt}</p>
          </div>
